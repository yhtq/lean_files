...
lemma card_G_nonzero : card G ≠ 0 := by
...
theorem only_sylow_subgroup_is_normal  {p: ℕ} [Fact (Nat.Prime p)] : (card (Sylow p G) = 1) -> ∀ (H : Sylow p G), (H: Subgroup G).Normal := by
  ...
  have : (H: Subgroup G).normalizer = ⊤ := by
    have : card ((H: Subgroup G).normalizer) = card G := by
      simp_rw [Subgroup.card_eq_card_quotient_mul_card_subgroup ((H: Subgroup G).normalizer: Subgroup G)]
      rw [<-h2]
      simp
...
theorem finite_subgroups_have_order_and_same_card_is_eq (a b: Subgroup G) (card_eq : card a = card b) (order: a ≤ b) : a = b := by
  ...
  set a':= (a: Set G)
  set b':= (b: Set G)
  apply Set.eq_of_subset_of_ncard_le order
  have : card a' = card b' := card_eq
  repeat rw [<-Set.Nat.card_coe_set_eq]
  rw [<-this]
...
def get_possible_sylow_groups_card (p: ℕ)  [is_prime: Fact (Nat.Prime p)] (q: ℕ) (factor: card G = p * q) (coprime: Coprime p q): possible_sylow_groups_card p q factor coprime:= by
  ...
  have : card (Sylow p G) ∈ possible_set := by
    unfold_let
    simp
    exact And.intro (⟨lt_succ_of_le lemma_le, Nat.card_ne_zero.mpr ⟨inferInstance, inferInstance⟩⟩) (And.intro pn1 lemma_q)

  exact {possible_set := possible_set, possible_set_def := by rfl, p := this}

-- 利用函数值构造类型上的划分
theorem card_eq_card_of_partition_by_func_fin {α β: Type} [Finite α] [Finite β] (f: α → β): card α = (∑ᶠ i: β, Set.ncard {a: α | f a = i}) := by
  simp_rw [<-Set.Nat.card_coe_set_eq]
  let dis_union_partition_fun : β -> Set α := λ i => {a: α | f a = i}
...
noncomputable def image_f [Finite α] [DecidableEq β] (f: α -> β)  : Finset β := Finset.image f Finset.univ

-- 上面定义使用集合值域的版本
theorem card_eq_card_of_partition_by_func_image {α β: Type} [Finite α] [DecidableEq β]  (f: α → β): card α = (∑ i ∈ image_f f, Finset.card ({a: α | f a = i}.toFinset)) := by
...
theorem card_of_sylow_p_group_when_p_exactly_divide_card_of_G (p: ℕ) [is_prime: Fact (Nat.Prime p)] (dvd: p ∣ card G) (exactly_dvd: ¬ p^2 ∣ card G) (G_p : Sylow p G) : card G_p  = p := by
  ...
  have card_ne_zero : card G ≠ 0 := Finite.card_ne_zero
  have : (card G).factorization p = 1 := exactly_dvd_fac_eq_one (card G) p (by
    rw [card_eq_fintype_card]
    exact card_ne_zero) dvd exactly_dvd
...
theorem number_of_p_order_ele_in_p_group (p: ℕ) [is_prime: Fact (Nat.Prime p)] (group_ord: card G = p) : card {a: G | orderOf a = p}.toFinset = p - 1 := by
  ...
  have group_ord' : card G = p := by rw [card_eq_nat_card, group_ord]
  have : image_f (orderOf : G -> ℕ) = {1, p} := by
    ...
    · intro h
      let ⟨y, hy⟩ := h
      have : orderOf y ∣ p := group_ord ▸ (orderOf_dvd_natCard y)
...
: card {a: G | orderOf a = p} = (p - 1) * (card (Sylow p G)) := by
  ...
  let generated_group_has_order_p (a: {a: G | orderOf a = p}) : card (a_subgroup a) = p ^ (card G).factorization p := by
    rw [exactly_dvd_fac_eq_one (card G) p (by
      rw [← card_eq_nat_card]
      exact Finite.card_ne_zero) dvd exactly_dvd]
    rw [pow_one]
    let ⟨a', ha⟩ := a
  ...
  have sylow_group_to_order_p_subset_card : ∀ s: Sylow p G, (sylow_group_to_order_p_subset s).card = p - 1 := by
    intro s
    unfold_let
    refine Eq.trans ?_ <| number_of_p_order_ele_in_p_group p <|
      card_of_sylow_p_group_when_p_exactly_divide_card_of_G p dvd exactly_dvd s
    simp_rw [← card_eq_card_toFinset, Set.mem_toFinset]
    rfl
...
theorem numbers_of_p_group_divide_into_orders {n: ℕ} {p: ℕ} [is_prime: Fact (Nat.Prime p)] (is_p_group: card G = p^n): p^n = ∑ i ∈ Finset.range (n+1), card {a: G | orderOf a = p^i} := by
  simp_rw [← card_eq_nat_card]
  rw [← card_eq_nat_card] at is_p_group
...
theorem have_exactly_numbers_of_elements_with_order_p_n_then_unique (p : ℕ) [is_prime: Fact (Nat.Prime p)]  :∑ i ∈ Finset.range (Nat.factorization (card G) p+1), card {a: G | orderOf a = p^i} ≤ p ^ (Nat.factorization (card G) p) -> card (Sylow p G) = 1 := by
  ...
  have non_trivial : Nontrivial (Sylow p G) := by
    ...
    let non_zero := @Finite.card_ne_zero (Sylow p G) _ _
    omega
  ...
  have p_sylow_card : ∀ s: (Sylow p G), card s = sylow_card := by
    intro s
    simp only [sylow_card, Sylow.card_eq_multiplicity, card_eq_nat_card]
  have sylow_is_p_group : ∀ s: (Sylow p G), card s = p ^ (Nat.factorization (card G) p) :=
    h_sylow_card ▸ p_sylow_card
  wlog h: ∃ a: G, a ∈ x ∧ a ∉ y
  ...
  · rcases h with ⟨a, hax, hay⟩
    ...
    have card_on_x_lt_card_on_G : ∀i ∈ sum_set, card {a: y | orderOf a = p ^ i} ≤ card {a: G | orderOf a = p ^ i} := by
      ...
      exact Finite.card_le_of_injective (f_inj_i i) (f_inj_is_inj i)
    ...
    have : card {a: y | orderOf a = p ^ k} < card {a: G | orderOf a = p ^ k} := by
      apply @Finite.card_lt_of_injective_of_not_mem _ _ _ _ (f_inj_i k) (f_inj_is_inj k) ⟨a, by simp; exact hk'⟩
      by_contra h_r
      simp at h_r
...
theorem not_simple_132 {G : Type} [Group G] [Finite G] (h : card G = 132) : ¬ IsSimpleGroup G := by
  by_contra cp
...
-- example {F : Type*} [Field F] [Finite F] [IsAlgClosed F] : False := by
--   let f: F[X] := ∏ i : F, (X - C i)
